...
export async function lookupFile(page, fileNumber, courtValue) {
  ...
  // Enter file number
  const fileInput = await page.$('input[type="text"]');
  ...
  // Click first Search button (File Number Search)
  const searchButtons = await page.$$('button[type="submit"]');
  let fileSearchBtn = null;
  ...
  if (!fileSearchBtn) throw new Error('Could not find File Number Search button');

  await fileSearchBtn.click();
